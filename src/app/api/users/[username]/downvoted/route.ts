...
export async function GET(
  ...
  try {
    ...
    // Only the profile owner can see their disliked posts (private, like Instagram)
    if (session.user.id !== user.id) {
      return NextResponse.json({ error: "Forbidden. You can only view your own disliked posts." }, { status: 403 });
    }
